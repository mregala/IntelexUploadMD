    ...
    public class SubLocation3ToIntelexDelete
    {
        ...
        public async Task SubLocation3ToIntelexDeleteFunc(/*ISubLocation1ExtractHelper _dataExtractHelper*/)
        {
            ...
            if (config != null)
            {
                ...
                string TargetWorksheetNameSubLocation3Delete = config.TargetWorksheetNameSubLocation3Delete;
                ...
                try
                {
                    ...
                    var worksheet = excelPackage.Workbook.Worksheets[TargetWorksheetNameSubLocation3Delete];
                        ...
                        while (worksheet.Cells[startRow, 1].Value != null)
                        {
                            ...
                            if (assetDataSet.EHSIncidentMang_csSubLocation3ObjectSetResult != null && assetDataSet.EHSIncidentMang_csSubLocation3ObjectSetResult.value.Count > 0) // EXIST!! so UPDATE!!
                            {
                                ...
                                if (subLocation3Values != null && subLocation3Values.Any())
                                {
                                    ...
                                    var response = DeleteDataToApi($"{subLoc3ApiEndpoint}({subLocation3Values.FirstOrDefault().Id})", payloadUpdate, apiUsername, apiPassword);

                                    //var payloadUpdate = assetDataSet.BuildSubLocation2Payload_Update(subLocation1Id, Name);
                                    //var jsonPayload = JsonConvert.SerializeObject(payloadUpdate, Formatting.Indented);
                                    //var response = DeleteDataToApi($"{subLoc2ApiEndpoint}({subLocation2Values.FirstOrDefault().Id})", payloadUpdate, apiUsername, apiPassword);
                    ...
                    {
                        Console.WriteLine($"Worksheet '{TargetWorksheetNameSubLocation3Delete}' not found in the Excel file.");
                    }
                }
                catch (Exception ex)
                {
